...
window.addEventListener('DOMContentLoaded',function(){
    var all=document.querySelectorAll('.canScroll');
    for(var i=0;i<all.length;i++){
        overscroll(all[i]);//.canScroll这个区域的都可以滚动 js中指定的类
    }
...
//获取url参数
function locationSearchParse(){
    //把下面的内容，去掉？转成对象，方便获取里面的参数
    //?from=meAccountSelectCard&userName=啊&kaNum=1
    var locurl=location.search;
    locurl=locurl.substring(1);//去掉?
    var arr=locurl.split('&');//按照&分割
    var locationObj={};
    for(var i=0;i<arr.length;i++){
        var str=arr[i];
        var temparr=str.split('=');//按照=分割，temparr里面只有两个元素
        locationObj[temparr[0]]=temparr[1];
    }
    return locationObj;
}
...
function telcenterStar(selectorstr){
    var tels=document.querySelectorAll(selectorstr);
    for(var i=0;i<tels.length;i++){
        var str=tels[i].innerHTML;
        var reg=/(\d{3})(\d{5})(\d{3})/;
        tels[i].innerHTML=str.replace(reg,'$1*****$3');
    }
}